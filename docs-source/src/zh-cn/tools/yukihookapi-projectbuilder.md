...
> 这是一个使用 `YukiHookAPI`作为核心的X曝光的模块自动构建工具。
> 
>实现自动化搜索依赖快速搭建一个包含X曝光的模块环境的安卓项目模板。

<IMGsrc="/images/yukihookapi-projectbuilder-zh-cn.png" 宽度="720"/>
...
若你想直接下载可以 [点击这里](https://github.com/HighCapable/YukiHookAPI-ProjectBuilder/releases)前往释放发布地址。
...
软件语言跟随当前系统，你可以在菜单栏**语言(语言)**一栏手动选择软件的界面语言。
